...
- 这份文档用于代码导航，重点是目录结构、模块依赖、数据流和设计理念摘要。
- 它回答“代码大致在哪、系统怎么串起来”，不逐项承担每个类的完整职责说明。
- 如果要看精确的职责边界和调用链，请转到 [`../ARCHITECTURE.md`](../ARCHITECTURE.md)。
- 如果要看项目约定与开发原则，请转到 [`../minirpg.md`](../minirpg.md)。
