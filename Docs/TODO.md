...
- 这份文档是问题清单，不是项目约定文档，也不是当前实现事实文档。
- 这里记录“还有哪些问题没有收敛”，不负责定义模块职责或开发原则。
- 如果某项技术债已经收敛到新的稳定边界，应该把结果同步写回 [`../ARCHITECTURE.md`](../ARCHITECTURE.md) 或 [`../minirpg.md`](../minirpg.md)，而不是只在这里打勾。
