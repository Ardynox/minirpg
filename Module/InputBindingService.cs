...
public sealed class InputBindingService
{
	...
	private static List<BindingActionDef> BuildDefaults()
	{
		var defs = new List<BindingActionDef>
		{
			...
			new("skill_prev", "input.action.skill_prev", InputBindingContext.Action, ":skill_prev", InputGesture.FromMouseWheel(MouseButton.WheelUp, alt: true), default),
			new("skill_next", "input.action.skill_next", InputBindingContext.Action, ":skill_next", InputGesture.FromMouseWheel(MouseButton.WheelDown, alt: true), default),
